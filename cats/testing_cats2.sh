...
do
	...
	then
		...
		python testing_tweets_new.py $FILE$j".csv" $DELIMITER $HEADER $DB $LANGUAGE $INIT_0 $MODE
	else
		# need init == 1 to do incremental insert
		python testing_tweets_new.py $FILE$j".csv" $DELIMITER $HEADER $DB $LANGUAGE $INIT_1 $MODE
	fi
done;
